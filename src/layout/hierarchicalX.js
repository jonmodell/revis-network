...
const Hierarchical1 = (data, options, persistent, screen, bounds, onStopped) => {
  const SCREEN = screen || { width: 600, height: 600 };
  const opts = options ? merge(defaultOpts, options) : { ...defaultOpts };
  const { nodeSpacing, direction, isDirected } = opts.settings;
  ...
  */
  ...
  const getWidth = node => {
    if (node) {
      const w = Math.max(1, node.children.reduce((acc, c) => acc + getWidth(c), 0));
      node.width = w;
      return w;
    }
    return 1;
  ...
  if (isDirected) {
    ...
    const crawl = n => {
      edges.forEach(edge => {
        ...
        let otherNode;
        if (edge.definition.from.toString() === nid && edge.definition.to !== edge.from) {
          otherNode = edge.end;
        }

        if (otherNode && otherNode.rank === null && otherNode.parent === undefined) {
          otherNode.parent = n;
    ...
    const crawl2 = n => {
      edges.forEach(edge => {
        const nid = n.id.toString();
        let otherNode;
        if (edge.definition.to.toString() === nid && edge.definition.to !== edge.from) {
  ...
  for (let i = 0; i < maxRank + 1; i++) {
    ...
    sameRankNodes.forEach(nd => {
      ...
      if (!nd.order) {
        ...
        nd.order = (nd.parent !== undefined ? nd.parent.order - (pw > 1 ? pw / 2 : 0) : 0) + ct;
